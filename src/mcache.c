...
////////////////////////////////////////////////////////////////////
// Cleansing for lineaddr = address/cache_line_size
////////////////////////////////////////////////////////////////////

void mcache_line_cleanse (MCache *c, Addr addr){
  Addr  tag  = addr; // full tags
  uns   set  = mcache_get_index(c,addr);
  uns   start = set * c->assocs;
  uns   end   = start + c->assocs;    
...
void mcache_vwq_cleanse (MCache *c, Addr addr){
  uns vwq_cleanse_radius = 8; 
  uns   offset = addr % vwq_cleanse_radius;
  Addr  start = addr - offset;
    
  for (uns ii=0; ii< vwq_cleanse_radius; ii++){
    mcache_line_cleanse(c, start+ii);
  }
}
