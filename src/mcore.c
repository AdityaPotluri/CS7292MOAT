...
#define DEFAULT_MEM_DELAY    900000
...
void mcore_cycle (MCore *c)
{
  ...
  for(uns ii=0; ii< CORE_WIDTH; ii++){
    ...
    if(c->inst_num >= c->trace_inst_num){
      ...
      if(c->trace_wb){
	if(mcache_mark_dirty(c->l3cache, orig_lineaddr)==FALSE){
	  if(MCORE_DO_WRITEBACKS){
	    if(c->id == DEBUG_CORE_ID){
	      printf("CORD-WBSEND for CoreID: %u ROBID: %u InstNum: %llu Cycle: %llu\n", c->id, MAX_UNS, c->inst_num, c->cycle);
	    }
	    memsys_access(c->memsys, orig_lineaddr, c->id, MAX_UNS, c->inst_num, TRUE);
	    mcache_vwq_cleanse(c->l3cache, orig_lineaddr); // virtual write queue
	  }
	}
      }
	...
	if( (L3_PERFECT==FALSE) && (l3outcome==MISS)){
	  ...
	  if(MCORE_DO_WRITEBACKS && c->l3cache->evicted_dirty_line){
	    ...
	    memsys_access(c->memsys, c->l3cache->evicted_line_addr, c->id, MAX_UNS, c->inst_num, TRUE);
	    mcache_vwq_cleanse(c->l3cache, c->l3cache->evicted_line_addr); // virtual write queue
	  }
	...
	printf("CORE-RDSEND for CoreID: %u ROBID: %u InstNum: %llu Cycle: %llu\n", c->id, myrobid, c->inst_num, c->cycle);
      ...
      if(memsys_access(c->memsys, orig_lineaddr, c->id, myrobid, c->inst_num, FALSE)==FAIL){
